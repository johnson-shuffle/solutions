...
Voter $i$'s ideal point can be expressed as:
$$
\theta_i = x_i^T \beta + \epsilon_i
$$
...
or equivalently:
$$
-\epsilon_i < \frac{(\theta_A ^ 2 - \theta_B ^ 2)}{(\theta_B - \theta_A)} + x_i^T \beta
$$
Relabeling and the disributional assumption for $\epsilon_i$ imply:
$$
P(\text{voter i chooses A}) = \Phi(\alpha + x_i^T \beta)
$$
